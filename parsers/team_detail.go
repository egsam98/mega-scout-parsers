...
func TeamDetail(teamUrl string) (*models.TeamDetail, error) {
	...
	if fetchHtmlErr != nil {
		return nil, errors.WithStack(fetchHtmlErr)
	}

	countryIdStr, exists := doc.Find("#land_select_breadcrumb > option").First().Attr("value")
	if !exists {
		return nil, errors.Errorf("%s: country is absent", teamUrl)
	}
...
func founded(teamUrl string) chan message.Message {
	...
	go func() {
		...
		if fetchHtmlErr != nil {
			future <- message.Error(errors.WithStack(fetchHtmlErr))
			return
		}
		...
		if err != nil {
			future <- message.Error(errors.Errorf("%s: %v", teamUrl, err))
			return
		}
