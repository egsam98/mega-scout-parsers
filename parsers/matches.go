...
func fetchMatchUrls(teamUrl string, seasonPeriod int, matchUrlsChan chan<- message.Message) {
	...
	if err != nil {
		matchUrlsChan <- message.Error(errors2.WithStack(err))
		return
	}
	...
	doc.Find("a.ergebnis-link").EachWithBreak(func(_ int, a *goquery.Selection) bool {
		if title, _ := a.Attr("title"); title == "Match report" {
			...
			if !exists {
				innerError = errors2.Errorf("href's absent")
				return false
			}
...
func matchInfo(matchUrl string, matchChan chan<- message.Message) {
	doc, err := utils.RetryFetchHtml(matchUrl, 10)
	if err != nil {
		matchChan <- message.Error(errors2.WithStack(err))
		return
	}
	...
	for i, result := range strings.Split(regexp.MustCompile(`\d:\d`).FindString(text), ":") {
		if result != "" && result != "-" {
			...
			if err != nil {
				matchChan <- message.Error(errors2.Errorf("%s: %v", matchUrl, err))
				return
			}
			scores[i] = &score
		}
	...
	doc.Find(".box > .large-6.columns").EachWithBreak(func(_ int, s *goquery.Selection) bool {
		...
		if err != nil {
			innerErr = errors2.Errorf("%s: %v", matchUrl, err)
			return false
		}
	...
	if !exists {
		matchChan <- message.Error(errors2.Errorf("%s: competition href is absent", matchUrl))
		return
	}
